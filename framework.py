...
Notes Application (similar to Iphone Notes)
'''
...
Lif yoU are using Min/max statements to check If a single number is or noT out of bounds thenY
use if statements
'''
'''
...
class Dropdown:
  ...
  def __init__(self,pos,size,up_color,down_color,text_color,captions,outPutCommand:Callable|None,maxy,rightClickCommand = None,tpos = (0,0),spacing:int = 1):
    self.pos = list(pos)
    ...
    self.mpos = [0,0]
    self.new_captions = []
    self.outPutCommand = outPutCommand if outPutCommand else lambda x : 0
    self.font = font.SysFont('Courier New', 20)
    self.yscroll = 0
    self.maxy = maxy
    self.tpos = tpos
    ...
    self.buttons = [Button((pos[0],pos[1]+count*size[1]*self.spacing),size[0],size[1],self.command,down_color,up_color,(min(up_color[0]+25,255),min(up_color[1]+25,255),min(up_color[2]+25,255)),caption,self.tpos[0],self.tpos[1],self.rightCommand,accepts_mb3=True) for count,caption in enumerate(self.new_captions)]
    self.offSetPos = (0,0)
  ...
  def offSetPos(self,newVal):
    self._offSetPos = newVal
    print(self.pos,newVal,self.size,self.maxy)
  ...
  def rightCommand(self):
    ...
    if self.spacing != 1: top +=1 
    bottom = self.size[1]*self.spacing
    print('successfully right clicked dd')
    ...
    if self._rect.collidepoint(mpos):  
      ...
      if self.size[1]*self.lenButtons >= self.maxy + self.yscroll and wheelState: #TODO: everything in this if statement can probably be optimized
        ...
        for button in self.buttons:
          ...
          button.update((mpos,mb1down,mb3down,[],mb1up))
      else:
        for button in self.buttons:
          button.update((mpos,mb1down,mb3down,[],mb1up))
    else:
...
class InputBox:
  ...
  def __init__(self,pos,size,caption = '',box_color = (100,100,100),max_chars=500,save_function:Callable|None = lambda x:x,restrict_input = None):
    self.pos = pos
    self.size = size
...
class ButtonSwitch:
  ...
  def __init__(self,pos:tuple[int,int],size:tuple[int,int],start_state:int,state_pics:list[Surface]):
    self.pos = pos
    self.size = size
    self.state = start_state
    self.state_pics = state_pics
    self._rect = Rect(self.pos,self.size)

  def onDown(self,state):
    pass

  def update(self,things:tuple[tuple[int,int],bool]):
    mpos,mb1 = things
    if self._rect.collidepoint(mpos) and mb1: 
  ...
  @offSetPos.setter
  def offSetPos(self,newVal):
    self._rect.left = self.pos[0]+ newVal[0]
    self._rect.top = self.pos[1]+ newVal[1]
    self._offSetPos = newVal

  def draw(self):
    screen.blit(self.state_pics[self.state],self._rect.topleft)
...
class Button:
  ...
  def __init__(self,pos,xlen,ylen,OnDownCommand:Callable|None,down_color,up_color,idle_color,text:str|Surface = Surface((0,0)),textx:int = 0,texty:int = 0,rightClickCommand:Callable|None = None,key:str|None = None,accepts_mb3:bool = False, OnUpCommand:Callable|None = None,keyCommand:str = 'OnDownCommand',text_color:tuple = (0,0,0)):
    self.x = pos[0]
    self.y = pos[1]
    self.xlen = xlen
    self.ylen = ylen
    self.OnDownCommand = OnDownCommand if OnDownCommand else lambda : 0
    self.OnUpCommand = OnUpCommand if OnUpCommand else lambda:0
    self.down_color = down_color
    self.up_color = up_color
    self.down = False
...
class MiniWindow:
  def __init__(self,name,pos,size,color =(70,70,70),exit_command:Callable = lambda:1):
...
class Window_Space:
    ...
    def addMiniWindow(self,name:str,pos:tuple,size:tuple,bg_color=None,exit_command:Callable|None=None) -> None:
      if bg_color == None:
        bg_color = (70,70,70)
    ...
    def topSize(self) -> int:
      try:
        return self._borders['top']._size[1] #type: ignore
      except AttributeError:
        return 0
    @property
    def rightSize(self) -> int:
      try:
        return self._borders['right']._size[0] #type: ignore
      except AttributeError:
        return 0
    @property
    def bottomSize(self) -> int:
      try:
        return self._borders['bottom']._size[1] #type: ignore
      except AttributeError:
        return 0 
    @mainSpace.setter
...
def loadSound(_FileName:str = '',usePath:bool = True) -> None:
  ...
  FileName = '\\'.join([PATH,"__Music",_FileName]) if usePath else _FileName
  mixer.music.unload()
  if _FileName:
